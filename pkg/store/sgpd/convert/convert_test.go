...
func TestEventRoundTrip(t *testing.T) {
			...
			Node: &sgp.Node{
				...
				Message: sgp.Message{User: &sgp.UserMessage{
					Parts: []sgp.ContentPart{
						{Text: &sgp.TextPart{Text: "what is 2+2?"}},
					},
				}},
			},
		},
	...
	for _, original := range cases {
		...
		if (got.SpawnedFrom == nil) != (original.SpawnedFrom == nil) {
			t.Errorf("SpawnedFrom nil mismatch")
		} else if original.SpawnedFrom != nil {
			if got.SpawnedFrom.SessionID != original.SpawnedFrom.SessionID {
				t.Errorf("SpawnedFrom.SessionID: got %q want %q", got.SpawnedFrom.SessionID, original.SpawnedFrom.SessionID)
			}
			if got.SpawnedFrom.NodeID != original.SpawnedFrom.NodeID {
				t.Errorf("SpawnedFrom.NodeID: got %q want %q", got.SpawnedFrom.NodeID, original.SpawnedFrom.NodeID)
			}
		}
		if (got.Node == nil) != (original.Node == nil) {
			t.Errorf("Node nil mismatch for event %q", original.Event)
		} else if original.Node != nil {
			assertNodeEqual(t, *original.Node, *got.Node)
		}
	}
}
	...
	original := sgp.Node{
		...
		Message: sgp.Message{User: &sgp.UserMessage{
			Parts: []sgp.ContentPart{
				...
				{Image: &sgp.ImagePart{BlobPart: sgp.BlobPart{MimeType: "image/png", Data: []byte{1, 2, 3}}}},
				{Audio: &sgp.AudioPart{BlobPart: sgp.BlobPart{MimeType: "audio/mp3", Data: []byte{4, 5}}}},
				{Video: &sgp.VideoPart{BlobPart: sgp.BlobPart{MimeType: "video/mp4", Data: []byte{6}}}},
				{File: &sgp.FilePart{BlobPart: sgp.BlobPart{MimeType: "application/pdf", Data: []byte{7}}, Name: "doc.pdf"}},
			},
		}},
	}
...
func assertNodeEqual(t *testing.T, want, got sgp.Node) {
	...
	if len(got.SynthesizedFrom) != len(want.SynthesizedFrom) {
		t.Errorf("Node.SynthesizedFrom len: got %d want %d", len(got.SynthesizedFrom), len(want.SynthesizedFrom))
	}
